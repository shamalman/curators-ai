...
import * as letterboxd from "./parsers/letterboxd.js";
import * as goodreads from "./parsers/goodreads.js";
import * as soundcloud from "./parsers/soundcloud.js";
import * as mixcloud from "./parsers/mixcloud.js";
import * as twitter from "./parsers/twitter.js";
import * as webpage from "./parsers/webpage.js";

const parsers = [spotify, appleMusic, googleMaps, youtube, letterboxd, goodreads, soundcloud, mixcloud, twitter];
...
const implementedParsers = new Set(["spotify", "apple-music", "youtube", "google-maps", "letterboxd", "goodreads", "soundcloud", "mixcloud", "twitter", "webpage"]);
...
export function detectSource(url) {
  ...
  // Fallback: any http/https URL → generic webpage parser (defuddle)
  // Every URL gets a chance to be parsed — no more "unsupported" gaps
  if (/^https?:\/\//i.test(url)) {
    return {
      supported: true,
      sourceType: webpage.sourceType,
      classification: webpage.classifyUrl(url) || "webpage",
