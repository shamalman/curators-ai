  ...
  try {
        ...
        const sections = subscribedIds
          ...
          .map(id => {
            ...
            const recLines = recs.map(r => {
              ...
              const recLink = r.slug ? ` [REC_LINK: /${handle}/${r.slug}]` : "";
              return `- ${r.title} (${r.category}) - "${ctx}"${tags}${recLink}`;
            });
    ...
    if (broaderRecs && broaderRecs.length > 0) {
      const lines = broaderRecs.map(r => {
        ...
        const handle = p?.handle || "unknown";
        const recLink = r.slug ? ` [REC_LINK: /${handle}/${r.slug}]` : "";
        return `@${p?.handle || "unknown"} (${p?.name || "Unknown"}): ${r.title} (${r.category})${ctx ? ` - "${ctx}"` : ""}${recLink}`;
      });
      broaderBlock = `\n\nBROADER NETWORK (other curators on the platform):\n---\n${lines.join("\n")}\n---`;
    }

    console.log(`[NETWORK_CONTEXT] profileId=${profileId} subscription_count=${subscribedIds.length} subscribed_rec_count=${subscribedRecs.length} broader_rec_count=${(broaderRecs || []).length}`);
    return subscribedBlock + broaderBlock;
  } catch (err) {
    console.error(`[NETWORK_CONTEXT_ERROR] profileId=${profileId} error=${err.message}`);
