...
export function newRecEmail({ curatorName, curatorHandle, recTitle, category, whyExcerpt, recUrl, unsubUrl }) {
  const categoryLabel = category ? `<p style="margin:0 0 4px 0;font-size:13px;color:#888;text-transform:uppercase;letter-spacing:0.05em;">${category}</p>` : '';
  const whyBlock = whyExcerpt ? `<p style="margin:16px 0 0 0;font-size:15px;color:#ccc;line-height:1.6;">${whyExcerpt}</p>` : '';

  const content = `
    <h2 style="margin:0 0 6px 0;font-size:20px;color:#fff;">New rec from ${curatorName}</h2>
    <p style="margin:0 0 20px 0;font-size:14px;color:#888;">@${curatorHandle}</p>
    ${categoryLabel}
    <p style="margin:0;font-size:18px;font-weight:600;color:#fff;">${recTitle}</p>
    ${whyBlock}
    <div style="margin:28px 0;">
      <a href="${recUrl}" style="background:#fff;color:#000;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;font-size:15px;">View rec</a>
    </div>
  `;

  return {
    subject: `New rec from ${curatorName}: ${recTitle}`,
    html: emailWrapper(content) + emailFooter(unsubUrl, 'https://curators.ai/settings', 'You received this because you subscribe to this curator on Curators.AI.')
  };
}
