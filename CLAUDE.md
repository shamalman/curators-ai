...
- Taste Timeline UI (`/me/timeline`). Data layer is ready; view not built.
...
Curator-only audit trail at `/me/timeline`. Shows every signal that shaped the taste profile in reverse chronological order, grouped by date.

**Entry point:** "How this was built →" link in Me tab below Stats section (TasteFileView.jsx line 275).

**Data sources:**
- `taste_confirmations` WHERE type IN ('taste_read_confirmed', 'correction') — confirmed and corrected inferences from taste reads
- `taste_read_ignores` — fetched by API but filtered out of UI (ignored signals not shown)
- `recommendations` WHERE profile_id = curator_id — curator's own recs (rec_is_own: true)

**Key files:**
- `app/api/timeline/route.js` — GET endpoint, cursor-paginated (50/page), merges three sources, parses taste_read source field format, joins taste_reads for article titles
- `app/(curator)/me/timeline/page.js` — auth-guarded page (note: .js not .jsx — project convention)
- `components/me/TasteTimeline.jsx` — timeline UI, uses T constants for theming, groups by local date, type labels (Taste read · confirmed/corrected, Your Recommendation, Saved Recommendation)

**Event types rendered:**
- confirmed (green dot): taste read inference confirmed
- corrected (amber dot): shows struck-through original + corrected text
- ignored: fetched but filtered from UI
- rec_saved / rec_is_own: true (blue dot, "Your Recommendation") — curator's own recs
- rec_saved / rec_is_own: false (purple dot, "Saved Recommendation") — deferred until saved_recs wired

**Taste profile stats:** `lib/taste-profile/generate.js` now includes "N taste signals confirmed or corrected" bullet in ## Stats section. `sources` jsonb now includes `taste_read_confirmed_count` and `taste_read_corrected_count`. Stats update on next rec save or manual POST /api/generate-taste-profile with { profileId }.

**Log marker:** [TIMELINE], [TIMELINE_ERROR]
...
- Taste Timeline: Saved Recommendation type (rec_is_own: false) deferred until saved_recs table is wired into timeline
