      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 16px", WebkitOverflowScrolling: "touch" }}>
        ...
        {filtered.map(function(item, i) { var ct = CAT[item.category] || CAT.other; var isArch = !!archived[item.id]; return (
          <div key={item.id} className={removing === item.id ? "rm" : "fu"} style={{ animationDelay: removing === item.id ? "0s" : (i * .03) + "s" }}>
            <div onClick={function() { if (!isArch) router.push('/find/' + (item.slug || item.id)); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 4px", borderBottom: "1px solid " + T.bdr, opacity: isArch ? 0.6 : 1, cursor: isArch ? "default" : "pointer" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: T.ink3, fontFamily: F, marginTop: 1 }}>{ct.label} {"·"} {fmtDate(item.date)}{item.visibility === "private" ? " · \uD83D\uDD12 Private" : ""}</div>
              </div>
              {isArch ? (
